...
import {
  ...
  Navigation, Package, ExternalLink,
} from "lucide-react";
import { format } from "date-fns";
import { QrScanner } from "@/components/qr-scanner";

type Tab = "scan" | "passengers" | "location" | "deliveries" | "incident";
...
export default function DriverPage() {
  ...
  const deliveries = useQuery(api.warehouse.listMyDeliveries);
  const acknowledgeDelivery = useMutation(api.warehouse.acknowledgeDelivery);
  const updateDeliveryStatus = useMutation(api.warehouse.updateDeliveryStatus);
  ...
  // UC-14: live location sharing
  const [sharingLocation, setSharingLocation] = useState(false);
  const [lastPing, setLastPing] = useState<{ lat: number; lng: number; speedKmh: number | null; at: number } | null>(null);
  ...
  // All derived-from-query values computed with safe fallbacks so hooks
  // below can depend on them without being declared after a conditional
  ...
  // Ping roughly every 15s while sharing is on — watchPosition fires far
  // more often than that, so throttle client-side rather than hammering
  // the mutation on every GPS tick.
  useEffect(() => {
    if (!sharingLocation || !selectedRoute || !navigator.geolocation) return;
    ...
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        pingLocation({ routeId: selectedRoute._id, latitude, longitude })
          .then((res) => setLastPing({ lat: latitude, lng: longitude, speedKmh: res.speedKmh, at: Date.now() }))
          .catch(() => {});
      },
  ...
  }, [sharingLocation, selectedRoute, pingLocation]);
  ...
  const handleAcknowledge = async (deliveryId: string) => {
    setDeliveryBusyId(deliveryId);
    setDeliveryError(null);
    try {
      await acknowledgeDelivery({ deliveryId: deliveryId as Id<"deliveries"> });
    } catch (err) {
      setDeliveryError(err instanceof Error ? err.message : "Failed to acknowledge delivery.");
    } finally {
  ...
  const handleMarkDelivered = async (deliveryId: string) => {
    ...
    setDeliveryError(null);
    try {
      await updateDeliveryStatus({ deliveryId: deliveryId as Id<"deliveries">, status: "delivered" });
    } catch (err) {
      setDeliveryError(err instanceof Error ? err.message : "Failed to update delivery.");
    } finally {
      setDeliveryBusyId(null);
    }
  };

  const pendingDeliveries = (deliveries ?? []).filter((d) => d.status === "pending" || d.status === "acknowledged");
  ...
  const tabs: { key: Tab; label: string; icon: React.ElementType; badge?: number }[] = [
    ...
    { key: "deliveries", label: "Deliveries", icon: Package, badge: pendingDeliveries.length || undefined },
  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-6 py-10 sm:px-10">
      <header className="mb-8 border-b border-slate-100 pb-8">
        ...
        <p className="mt-2 text-sm text-slate-500">Scan boarding passes, share your live location, manage deliveries, and report incidents.</p>
      </header>
      ...
      <div className="mb-8 flex flex-wrap gap-1 rounded-2xl bg-slate-100 p-1">
        {tabs.map(({ key, label, icon: Icon, badge }) => (
          ...
          >
            ...
            {label}
            {!!badge && <span className="rounded-full bg-rose-500 px-1.5 py-0.5 text-[9px] font-black text-white">{badge}</span>}
          </button>
        ))}
      </div>
      ...
      {tab === "location" && (
        <div className="space-y-6">
          <h2 className="text-xl font-black text-slate-950 flex items-center gap-2">
            <Navigation className="h-5 w-5 text-cyan-600" /> Live Location — {selectedRoute?.name ?? "No route"}
          </h2>

          <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm max-w-lg">
            <p className="mb-4 text-xs text-slate-500">
              Turn this on while driving your route — parents and the transport admin will see your bus's live position update automatically.
            </p>
            <button
              onClick={() => setSharingLocation((v) => !v)}
              disabled={!selectedRoute}
              className={`w-full rounded-2xl py-3 text-sm font-black text-white transition disabled:opacity-50 ${sharingLocation ? "bg-rose-600 hover:bg-rose-700" : "bg-cyan-600 hover:bg-cyan-700"}`}
            >
              {sharingLocation ? "Stop Sharing Location" : "Start Sharing Location"}
            </button>

            {sharingLocation && (
              <div className="mt-4 flex items-center gap-2 text-xs font-bold text-cyan-700">
                <Navigation className="h-3.5 w-3.5 animate-pulse" /> Sharing live — pings every ~15s
              </div>
            )}

            {lastPing && (
              <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-xs text-slate-600 space-y-1">
                <p><span className="font-bold">Last position:</span> {lastPing.lat.toFixed(5)}, {lastPing.lng.toFixed(5)}</p>
                {lastPing.speedKmh != null && <p><span className="font-bold">Speed:</span> ~{lastPing.speedKmh} km/h</p>}
                <p className="text-slate-400">Updated {format(lastPing.at, "HH:mm:ss")}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {tab === "deliveries" && (
        <div className="space-y-6">
          <h2 className="text-xl font-black text-slate-950 flex items-center gap-2">
            <Package className="h-5 w-5 text-amber-600" /> My Deliveries
          </h2>

          {deliveryError && (
            <p className="rounded-2xl bg-rose-50 px-4 py-2.5 text-xs font-bold text-rose-700">{deliveryError}</p>
          )}

          {(deliveries ?? []).length === 0 ? (
            <div className="flex flex-col items-center rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50 p-16 text-center">
              <Package className="mb-4 h-12 w-12 text-slate-300" />
              <p className="font-bold text-slate-500">No deliveries assigned.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {(deliveries ?? []).map((d) => (
                <div key={d._id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm flex items-center justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-black uppercase ${
                        ...
                        : d.status === "acknowledged" ? "bg-sky-50 text-sky-700"
                        : d.status === "delivered" ? "bg-emerald-50 text-emerald-700"
                        : "bg-slate-100 text-slate-500"
                      }`}>{d.status}</span>
                      <h3 className="font-black text-slate-950 text-sm">{d.quantity}x {d.itemName}</h3>
                    </div>
                    <p className="text-xs text-slate-500">For: {d.recipientLabel}{d.notes ? ` · ${d.notes}` : ""}</p>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    {d.status === "pending" && (
                      <button
                        onClick={() => handleAcknowledge(d._id)}
                        disabled={deliveryBusyId === d._id}
                        className="rounded-2xl bg-sky-600 px-4 py-2 text-[10px] font-black text-white hover:bg-sky-700 disabled:opacity-50"
                      >
                        Acknowledge Receipt
                      </button>
                    )}
                    {d.status === "acknowledged" && (
                      <button
                        ...
                        disabled={deliveryBusyId === d._id}
                        className="rounded-2xl bg-emerald-600 px-4 py-2 text-[10px] font-black text-white hover:bg-emerald-700 disabled:opacity-50"
                      >
                        Mark Delivered
                      </button>
                    )}
