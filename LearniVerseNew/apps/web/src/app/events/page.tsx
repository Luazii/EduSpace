...
import {
  ...
  QrCode, Clock, CreditCard, User, Mail
} from "lucide-react";
...
function GuestEventCard({
  ...
  const handleGet = async () => {
    setError(null);
    if (!guestName.trim() || !emailValid) {
      setError("Enter your name and a valid email above first.");
      return;
    }
    setLoading(true);
    try {
      if (event.ticketPrice && event.ticketPrice > 0) {
        const { authorizationUrl } = await initGuestCheckout({
          eventId: event._id,
          guestName,
          guestEmail,
          origin: window.location.origin,
  ...
  return (
    <div className={`rounded-3xl border bg-white p-6 shadow-sm transition hover:shadow-md ${owned ? "border-emerald-200 ring-2 ring-emerald-100" : "border-slate-200"}`}>
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <h3 className="text-base font-black text-slate-950">{event.title}</h3>
          <p className="mt-1 text-xs text-slate-500 leading-relaxed line-clamp-2">{event.description}</p>
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col px-6 py-10 sm:px-10">
      ...
      <section className="mb-12">
        ...
        {upcomingEvents.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {upcomingEvents.map((event) => (
              <GuestEventCard key={event._id} event={event} guestName={guestName.trim()} guestEmail={guestEmail} />
            ))}
          </div>
        )}
      </section>
...
export default function EventsPage() {
  ...
  return isAuthenticated ? <SignedInEventsPage /> : <GuestEventsPage />;
}
