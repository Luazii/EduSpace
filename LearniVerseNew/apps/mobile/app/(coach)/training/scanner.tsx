...
export default function CoachScannerScreen() {
  ...
  const markAttendanceByQR = useMutation(api.sports.markAttendanceByQR);
  ...
  async function handleBarCodeScanned({ data }: { type: string; data: string }) {
    ...
    try {
      ...
      setResult({
        success: true,
        message: `Attendance marked for ${response?.fullName ?? "Athlete"}!`,
      });
    } catch (error) {
      setResult({
        success: false,
        ...
        >
          <View className="flex-1 bg-black/40 p-5">
            <View className="flex-row items-center justify-between pt-12 mb-8">
              ...
              <Text className="text-white font-black text-lg tracking-tight">Scan Athlete ID</Text>
            ...
            {result && (
              <View className={`p-6 rounded-3xl mb-8 ${result.success ? 'bg-emerald-600' : 'bg-rose-600'}`}>
                ...
                >
                  <Text className="text-white font-bold">Scan Next Athlete</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
