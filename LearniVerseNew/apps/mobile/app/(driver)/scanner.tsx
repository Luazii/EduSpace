...
export default function DriverScannerScreen() {
  ...
  const { routeId, scanType } = useLocalSearchParams<{ routeId: string; scanType: "board" | "dropoff" }>();
  const [permission, requestPermission] = useCameraPermissions();
  const scanBusPassengerByQR = useMutation(api.transport.scanBusPassengerByQR);
  ...
  async function handleBarCodeScanned({ data }: { type: string; data: string }) {
    ...
    try {
      ...
      setResult({
        success: true,
        message: `${scanType === "board" ? "Boarded" : "Dropped off"} ${response?.fullName ?? "student"}!`,
      });
    } catch (error) {
      setResult({
        success: false,
        message: error instanceof Error ? error.message : "Invalid scan.",
      });
    } finally {
      setLoading(false);
    }
        ...
        >
          <View className="flex-1 bg-black/40 p-5">
            <View className="flex-row items-center justify-between pt-12 mb-8">
              ...
              <Text className="text-white font-black text-lg tracking-tight">Scan ID Card</Text>
            ...
            <View className="flex-1 items-center justify-center">
              ...
              <Text className="text-white/70 text-center mt-6 font-medium">
                {scanType === 'board' ? 'Scan ID to board bus' : 'Scan ID to drop off'}
              </Text>
            </View>
            ...
            {result && (
              <View className={`p-6 rounded-3xl mb-8 ${result.success ? 'bg-emerald-600' : 'bg-rose-600'}`}>
                ...
                >
                  <Text className="text-white font-bold">Scan Next Student</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
